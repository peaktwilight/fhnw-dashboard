        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="flex gap-2">
            ...
            <motion.button
              ...
              className="px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
            >
              {loading ? (
                <>
      ...
      <AnimatePresence>
        {showTokenInput && (
          <motion.div
            ...
            className="border-b border-slate-200 dark:border-slate-700 p-6"
          >
            <div className="space-y-4">
              <div>
                ...
                <div className="text-sm text-slate-500 dark:text-slate-400 space-y-1 mb-4">
                  <p>1. Go to <a href="https://auxilium.webapps.fhnw.ch/" target="_blank" rel="noopener noreferrer" className="text-orange-500 hover:underline">auxilium.webapps.fhnw.ch</a></p>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-slate-900 dark:text-white focus:ring-2 focus:ring-orange-500"
                />
                <button
                  onClick={handleSaveToken}
                  disabled={!tokenInput.trim()}
                  className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 disabled:bg-slate-400 disabled:cursor-not-allowed transition-colors"
                >
      ...
      <div className="p-6">
        ...
        {!token && (
          ...
          >
            ...
            <p className="text-slate-500 dark:text-slate-400">{getTranslation('no_token')}</p>
          </motion.div>
        )}
              ...
              <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                ...
                <tbody className="bg-white dark:bg-slate-800 divide-y divide-slate-200 dark:divide-slate-700">
                  {anmeldungen.map((anmeldung) => (
                    <tr key={anmeldung.anmeldungId} className="hover:bg-slate-50 dark:hover:bg-slate-700">
        ...
        {anmeldungen.length === 0 && !loading && !error && token && (
          ...
          >
            ...
            <p className="text-slate-500 dark:text-slate-400">{getTranslation('no_data')}</p>
          </motion.div>
        )}
      </div>
    </motion.div>
