      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <motion.div
          ...
          className={`relative p-4 rounded-lg border cursor-pointer transition-all ${
            locale === 'en'
              ? 'border-orange-500 bg-gradient-to-br from-orange-50 to-white dark:from-orange-900/20 dark:to-slate-800'
              : 'border-slate-200 dark:border-slate-700 hover:border-orange-500 hover:shadow-sm'
          }`}
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className={`flex items-center justify-center h-8 w-8 rounded-md ${
                locale === 'en' ? 'bg-orange-100 dark:bg-orange-900/50' : 'bg-slate-100 dark:bg-slate-800'
              }`}>
                <span className={`font-semibold text-sm ${
                  locale === 'en' ? 'text-orange-500 dark:text-orange-400' : 'text-slate-500 dark:text-slate-400'
                }`}>EN</span>
              </div>
              <div className="ml-3">
                <h3 className="font-medium text-slate-900 dark:text-white">English</h3>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              <div className={`flex items-center justify-center h-8 w-8 rounded-md ${
                locale === 'de' ? 'bg-orange-100 dark:bg-orange-900/50' : 'bg-slate-100 dark:bg-slate-800'
              }`}>
                <span className={`font-semibold text-sm ${
                  locale === 'de' ? 'text-orange-500 dark:text-orange-400' : 'text-slate-500 dark:text-slate-400'
                }`}>DE</span>
              </div>
              <div className="ml-3">
                <h3 className="font-medium text-slate-900 dark:text-white">Deutsch</h3>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
